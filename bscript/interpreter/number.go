...
func makeScriptNumber(bb []byte, scriptNumLen int, requireMinimal, afterGenesis bool) (*scriptNumber, error) {
	...
	v := new(big.Int)
	for i, b := range bb {
		v.Or(v, new(big.Int).Lsh(new(big.Int).SetBytes([]byte{b}), uint(8*i))) //nolint:gosec // loop index bounded by bb length
	}
	...
	if bb[len(bb)-1]&0x80 != 0 {
		...
		shift.Not(shift.Lsh(shift, uint(8*(len(bb)-1)))) //nolint:gosec // bb length bounded by protocol
		v.And(v, shift).Neg(v)
	}
	return &scriptNumber{
		val:          v,
