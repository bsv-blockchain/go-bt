...
func TestStack(t *testing.T) {
	...
	for _, test := range tests {
		...
		if int32(len(test.after)) != s.Depth() { //nolint:gosec // test data length is safe to convert
		...
		for i := range test.after {
			val, err := s.PeekByteArray(s.Depth() - int32(i) - 1) //nolint:gosec // loop index is safe to convert
			if err != nil {
