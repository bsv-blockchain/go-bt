...
func TestTx_Change(t *testing.T) {
	...
	t.Run("determine fees are correct, correct change given", func(t *testing.T) {
		...
		// pay to
		require.NoError(t, tx.PayToAddress("1C8bzHM8XFBHZ2ZZVvFy2NSoAZbwCXAicL", 500))

		// add some op return
		require.NoError(t, tx.AddOpReturnPartsOutput([][]byte{[]byte("hi"), []byte("how"), []byte("are"), []byte("you")}))

		require.NoError(t, tx.ChangeToAddress("1D7gaZJo3vPn2Ks3PH694W9P8UVYLNh2jY", fq))
		...
		assert.InEpsilon(t, 0.49800796812749004, feeRate, 0.0001)
	})
	...
	t.Run("multiple Inputs, spend all", func(t *testing.T) {
		...
		signAllInputs(t, tx, "5JXAjNX7cbiWvmkdnj1EnTKPChauttKAJibXLm8tqWtDhXrRbKz")
	...
	tests := map[string]struct {
		...
		"no change to add should return no change output": {
			tx: func() *bt.Tx {
				...
				require.NoError(t, tx.PayToAddress("mxAoAyZFXX6LZBWhoam3vjm6xt9NxPQ15f", 1000))
				return tx
			}(),
			index:           0,
			fees:            fq,
