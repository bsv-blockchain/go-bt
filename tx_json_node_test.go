...
func TestTxJSON_Node_JSON(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			tx := &bt.Tx{}
			require.NoError(t, json.Unmarshal(bb, tx.NodeJSON()))
...
func TestTxsJSON_Node(t *testing.T) {
	...
	nTxs := make(bt.Txs, 0)
	require.NoError(t, json.Unmarshal(bb, nTxs.NodeJSON()))

	assert.Len(t, txs, len(nTxs))
	for i, tx := range txs {
...
func TestOutput_Node_UnmarshalJSON(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			output := &bt.Output{}
			require.NoError(t, json.Unmarshal([]byte(test.json), output.NodeJSON()))
...
func TestOutput_UnmarshalJSON(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			output := &bt.Output{}
			require.NoError(t, json.Unmarshal([]byte(test.json), output))
