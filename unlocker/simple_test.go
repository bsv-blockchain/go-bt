...
func TestLocalUnlocker_ValidSignature(t *testing.T) {
	tests := map[string]struct {
		...
		"valid signature 1": {
			tx: func() *bt.Tx {
				tx := bt.NewTx()
				require.NoError(t, tx.From("45be95d2f2c64e99518ffbbce03fb15a7758f20ee5eecf0df07938d977add71d", 0, "76a914c7c6987b6e2345a6b138e3384141520a0fbc18c588ac", 15564838601))

				script1, err := bscript.NewFromHexString("76a91442f9682260509ac80722b1963aec8a896593d16688ac")
				require.NoError(t, err)

				require.NoError(t, tx.AddP2PKHOutputFromScript(script1, 375041432))

				script2, err := bscript.NewFromHexString("76a914c36538e91213a8100dcb2aed456ade363de8483f88ac")
				require.NoError(t, err)

				require.NoError(t, tx.AddP2PKHOutputFromScript(script2, 15189796941))
		...
		"valid signature 2": {
			tx: func() *bt.Tx {
				...
				require.NoError(t, tx.AddP2PKHOutputFromScript(script1, 2200000000))

				script2, err := bscript.NewFromHexString("76a9143ac52294c730e7a4e9671abe3e7093d8834126ed88ac")
				require.NoError(t, err)

				require.NoError(t, tx.AddP2PKHOutputFromScript(script2, 2799998870))
				return tx
			}(),
		},
	}
...
func TestLocalUnlocker_NonSignature(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			tx := test.tx
			assert.Len(t, tx.Inputs, len(test.expUnlockingScripts))
